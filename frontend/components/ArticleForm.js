...
export default function ArticleForm(props) {
  ...
  const { setCurrentArticleId, currentArticleId ,postArticle, updateArticle ,articles} = props
  ...
  useEffect(() => {
    ...
    const articleToEdit = getArticleById(currentArticleId)
    if(articleToEdit) {
     setValues({
  ...
  const onSubmit = evt => {
    ...
    if (currentArticleId) {
      updateArticle({article_id: currentArticleId, article: values}).then(()=> setValues(initialFormValues))
    } else {
      postArticle(values)
      setValues({
        title: '',
  ...
  function getArticleById(articleId) {
   return articles.find((article) => article.article_id === articleId )
